...
func (g *JavaGen) genJNIFuncBody(o *types.Func, sName string, jm *java.Func, isjava bool) {
  ...
  for i := res.Len() - 1; i >= 0; i-- {
    t := res.At(i).Type()
    if !isErrorType(t) {
      g.Printf("LOG_INFO(\"end of function %s\");\n", sName)
...
func (g *JavaGen) genMethodInterfaceProxy(oName string, m *types.Func) {
  ...
  g.Indent()
  g.Printf("//dan: genMethodInterfaceProxy\n")
...
func (g *JavaGen) GenC() error {
  ...
  g.Indent()
  g.Printf("//dan: GenC pkgName:%s\n", pkgName)

  g.Printf("LOG_INFO(\"GenC():%s\");\n", pkgName)

  g.Printf("jclass clazz;\n")

  g.Printf("LOG_INFO(\"env is %%p\",env);\n")
  for _, s := range g.structs {
    ...
    g.Printf("LOG_INFO(\"start of init() %s\");\n", g.className())
